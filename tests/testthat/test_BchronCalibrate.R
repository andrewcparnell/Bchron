...
test_that("plot.BchronCalibrate works", {
  ...
  p <- plot(ages3)
  expect_doppelganger("calibration_with_positions", p)
  p <- plot(ages4)
  expect_doppelganger("calibration_new_calcurve", p)
  p <- plot(ages1, includeCal = TRUE)
  expect_doppelganger("calibration_includeCal", p)
  expect_error(plot(ages3, includeCal = TRUE))
  p <- plot(ages5, includeCal = TRUE)
  expect_doppelganger("calibration_includeCal_new_calcurve", p)
