...
You can control the number of positions reported with `numPos`. For example you could now add this to the plot with
```{r}
...
Finding the age of maximum influence is a more computationally intensive procedure and involves running the Bchron model repeatedly, leaving each date out in turn, and quantifying the effect on the chronology of removing each date. The function is called `dateInfluence`.
