...
coreInfluence.BchronologyRun = function(bchrRun1,
  ...
  # Final diffs 
  diffs = data.frame(positions = usePositions,
  ...
  if('max' %in% type) {
    ...
    df2 = with(diffs, data.frame(positions = positions[positionsAll],
    ...
    if('plot' %in% type) {
      with(df2, polygon(c(ages, rep(ageTolerance, length(ages))), c(positions, rev(positions)), col = 'gray',
                        border = NA))
    }
  }
